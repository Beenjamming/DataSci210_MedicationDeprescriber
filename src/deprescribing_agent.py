...
class DeprescribingAgent:
    ...
    def __init__(self, groq_key, data_path, logger):
        ...
        self.llm_agent = llmAgent(groq_key=groq_key, data_path=data_path)
...
class SearchPatientHistory:
    ...
    def search_encounter_source(self):
        ...
        for recommendation_str, diagnosis_list in self.recommendation_dict.items():
            ...
            # extract information
            encounters_dict, encounters_token_count = (
                self.llm_agent.extract_without_RAG(
            ...
            # add the data source to the diagnosis dict
            encounters_dict["source"] = "Patient encounter record"
            # add the recommendation and associated diagnoses
            encounters_dict["recommendation considered"] = recommendation_str
            encounters_dict["associated diagnosis list"] = self.recommendation_dict[
                recommendation_str
            ]

            # store the bool, explanation, info source, recomendation and associated diagnoses
            self.search_history_thus_far_list.append(encounters_dict)
            # store token count
            ...
            self.logger.info(
                f" -  encounters_source_bool: {encounters_dict["diagnosis_boolean"]}"
            )
            self.logger.info(f" -  encounters_dict: {encounters_dict}")
