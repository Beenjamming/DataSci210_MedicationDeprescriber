...
def get_notes(notes_rm_debug=False):
    '''
    ...
    notes = notes.merge(le_df[['key','PtDischargeDate']], left_on='EncounterKey', right_on='key')
    ...
    if notes_rm_debug:
        ...
        rm_notes['PtDischargeDate'] = pd.to_datetime(rm_notes['PtDischargeDate'], errors='coerce')
        rm_notes['NoteDate'] = pd.to_datetime(rm_notes['NoteDate'], errors='coerce')

        # Calculate the difference while handling missing values
        rm_notes['dis-note_vs_dis-date'] = rm_notes['PtDischargeDate'] - rm_notes['NoteDate']

        rm_notes.to_csv('Data/removed_discharged_notes.csv', index=False)
